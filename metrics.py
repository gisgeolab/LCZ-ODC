import numpy as np
from scipy import ndimage, ndimage, misc
import cv2
...
def ergas(img_fake, img_real, scale=6):
    img_fake = np.moveaxis(img_fake, -1, 0)
    img_real = np.moveaxis(img_real, -1, 0)
...
def D_lambda(img_fake, img_lm, block_size=32, p=1):
    ...
    # img_fake = np.moveaxis(img_fake, -1, 0)
    # img_lm = np.moveaxis(img_lm, -1, 0)
    img_fake = np.transpose(img_fake, (1, 2, 0))
    img_lm = np.transpose(img_lm, (1, 2, 0))
