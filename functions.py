...
def plot_interactive_spectral_sign_train_test(wvl, spectral_sign, spectral_sign_test, legend):
    
    fig = go.Figure()
    for LCZ in sorted(list(legend.keys())):
        fig.add_trace(go.Scatter(x = sorted(wvl),
                                y=spectral_sign[LCZ],
                                name = f"TRAIN - {legend[LCZ][0]}",
                                line_color = legend[LCZ][1]))
        fig.add_trace(go.Scatter(x = sorted(wvl),
                                y=spectral_sign_test[LCZ],
                                name = f"TEST - {legend[LCZ][0]}",
                                line=dict(color=legend[LCZ][1], dash='dash')))
    fig.update_xaxes(title_text = "Wavelength (nm)")
    fig.update_yaxes(title_text = "Reflectance")
    fig.update_layout(width = 1000, height = 600, title = 'Median spectral signature of the training and testing samples')
...
def testing_area(sel_prisma_date, legend):
...
def parameter_tuning(classification_method, X_train, y_train):
    ...
    if classification_method == 'RandomForest':
        ...
        param_grid = {
            'n_estimators': [100, 150, 200, 500], #[100, 150, 200, 500]
...
def print_accuracy(classification_method, sel_prisma_date, legend):
    
    with rasterio.open(f'./classified_images/classified_{classification_method}_' + sel_prisma_date.replace('-', '') + '_medianfilter_30m.tif') as src:
        mappa = src.read()
        modified_mappa = mappa.copy()
        modified_mappa[modified_mappa == 0] = np.nan
        unique_values_mappa = np.unique(modified_mappa)
        
    with rasterio.open('./layers/testing_samples/testing_set_' + sel_prisma_date.replace('-', '') + '_30m.tif') as src:
        test = src.read()
...
# Function used in 6 - Validation.ipynb
import plotly.express as px
def print_accuracy_s2(classification_method, sel_s2_date, prisma_s2_dates, legend):
    with rasterio.open(f'./classified_images/comparison_PRISMA_S2/classified_{classification_method}_' + sel_s2_date.replace('-', '') + 
                       '_medianfilter_20m_align_S2_masked.tif') as src:
        mappa = src.read()
        modified_mappa = mappa.copy()
        modified_mappa[modified_mappa == 0] = np.nan
        unique_values_mappa = np.unique(modified_mappa)
    
    with rasterio.open('./layers/testing_samples/testing_set_' + prisma_s2_dates[sel_s2_date].replace('-', '') + '_20m.tif') as src:
        test = src.read()
        modified_test = test.astype('float')
        modified_test[modified_test == 0] = np.nan
        unique_values_test = np.unique(modified_test)
...
import plotly.express as px
def print_accuracy_lczgenerator(classified_image, sel_prisma_date, legend):
    ...
    with rasterio.open('./layers/testing_samples/testing_set_' + sel_prisma_date.replace('-', '') + '_10m.tif') as src:
        test = src.read()
        modified_test = test.astype('float')
        modified_test[modified_test == 0] = np.nan
        unique_values_test = np.unique(modified_test)
