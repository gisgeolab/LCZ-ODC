...
def import_training_samples():
  ...
  file_basepath = '/home/asi/data/layers/training_samples'
  list_files = sorted(list(iglob(join(file_basepath, '*.tif*'), recursive=True)))

  file_names = []
  for path in list_files:
      file_name = path.split('/')[-1]
      file_name = file_name.replace('_30m.tif', '')
  ...
  key = get_key_from_value(file_dict, '/home/asi/data/layers/training_samples/training_set_20230209_30m.tif')

  list_files = sorted(list(iglob(join(file_basepath, '*.tif*'), recursive=True)))

  p_name = "training_samples"
  text_p0 = """name: """ + p_name + """
description: Training Samples for LCZ classification
metadata_type: eo3
...
measurements:""")
  ...
  print(colored('All training samples products have been updated into the ODC','green'))
...
id: 40000000-0000-0000-0000-0000"""+ str(date_uuid) +"""
...
properties:
  ...
  print(colored('All training samples products have been imported into the ODC','green'))
